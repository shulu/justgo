...
func GetInvoices() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error ocurred while listing food"})
		}
...
func GetInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "errpr occur while fetching the order"})
		}
		...
		invoiceView.Table_number = allOrderItems[0]["table_num"]
...
func UpdateInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err := orderCollection.FindOne(ctx, bson.M{"invoice_id": invoiceId}).Decode(&order)
		defer cancel()
